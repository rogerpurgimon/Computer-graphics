...
{
	//necessitem 4 operacions per a llegir la matriu
	//cada condició és necessària perquè es pugui llegir en totes les direccions.
	if (y2 < y1 && x2 < x1) {
		for (int y = y2; y < y1; y++) {
			for (int x = x2; x < x1; x++) {
				if (y == (int)(y1 + ((y2 - y1) * (x - x1) / (x2 - x1)))) {
					setPixel(x, y, color);
				}
			}
		}
